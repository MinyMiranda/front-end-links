  ...
  >
    <form id="form-link">
      <div class="mb-3">
        ...
        <input id="link_id" v-model="form.link_id" type="hidden" name="link_id" />
        <label for="url" class="form-label">Url</label>
        <input type="text" name="url" v-model="form.url" class="form-control" />
        <label for="count" class="form-label">Numero de acessos permitidos</label>
        <input type="text" name="count" v-model="form.count" class="form-control" />
      </div>
    </form>
  </modal-component>
</template>
...
export default {
  ...
  mounted: async function () {
    this.form.link_id = window.location.search.substring(1);
    await this.list();
  },
  methods: {
    async list() {
      try {
        let { data } = await api.get("/api/redirects/" + this.form.link_id);
        console.log(data);
        this.links = data;
      } catch (error) {
        console.error(error);
    ...
    showModalEdit(id) {
      ...
      this.form.count = this.links[id].count;
      this.create = false;
      this.showModal = true;
    },
    async createLink() {
      ...
      try {
        await api.post("/api/redirects/store", formData);
        this.$swal({
          icon: "success",
          timer: 3000,
    ...
    async confirmDelete(id) {
      try {
        await api.delete("/api/redirects/delete/" + id);
        this.$swal({
          icon: "success",
          timer: 3000,
        ...
        this.$swal({
          icon: "error",
          html: "Erro ao deletar, entre em contato com o suporte.",
        });
      }
    },
  },
